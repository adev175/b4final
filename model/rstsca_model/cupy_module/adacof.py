...
def cupy_kernel(strFunction, intFilterSize, intDilation, objectVariables):
    strKernel = globals()[strFunction]      # returns the dictionary of the current global symbol table.

    while True:
        objectMatch = re.search('(SIZE_)([0-4])(\()([^\)]*)(\))', strKernel)
        # objectMatch contains 5 groups, divided by '()'
        ...
        intSizes = objectVariables[strTensor].size()    # B, C, H, W
...
class FunctionAdaCoF(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, offset_i, offset_j, dilation):
        ...
        if input.is_cuda == True:
            ...
            n = output.nelement()   # B*C*Hout*Wout
            cupy_launch('kernel_AdaCoF_updateOutput',
                        cupy_kernel('kernel_AdaCoF_updateOutput', intFilterSize, dilation, {
                            'input': input,
                            'weight': weight,
                            'offset_i': offset_i,
                            'offset_j': offset_j,
                ...
                block=tuple([512, 1, 1]),
                args=[n, input.data_ptr(), weight.data_ptr(), offset_i.data_ptr(), offset_j.data_ptr(),
                      output.data_ptr()],
                stream=Stream
            )
    ...
    def backward(ctx, gradOutput):
        ...
        gradInput = input.new_zeros(intSample, intInputDepth, intInputHeight, intInputWidth) if ctx.needs_input_grad[
                                                                                                    0] == True else None
        gradWeight = input.new_zeros(intSample, intFilterSize ** 2, intOutputHeight, intOutputWidth) if \
        ctx.needs_input_grad[1] == True else None
        gradOffset_i = input.new_zeros(intSample, intFilterSize ** 2, intOutputHeight, intOutputWidth) if \
        ctx.needs_input_grad[2] == True else None
        gradOffset_j = input.new_zeros(intSample, intFilterSize ** 2, intOutputHeight, intOutputWidth) if \
        ctx.needs_input_grad[2] == True else None
        ...
        if input.is_cuda == True:
            ...
            cupy_launch('kernel_AdaCoF_updateGradWeight',
                        cupy_kernel('kernel_AdaCoF_updateGradWeight', intFilterSize, dilation, {
                            'gradLoss': gradOutput,
                            'input': input,
                            'offset_i': offset_i,
                            'offset_j': offset_j,
                ...
                block=tuple([512, 1, 1]),
                args=[n_w, gradOutput.data_ptr(), input.data_ptr(), offset_i.data_ptr(), offset_j.data_ptr(),
                      gradWeight.data_ptr()],
                stream=Stream
            )
            ...
            cupy_launch('kernel_AdaCoF_updateGradAlpha',
                        cupy_kernel('kernel_AdaCoF_updateGradAlpha', intFilterSize, dilation, {
                            'gradLoss': gradOutput,
                            'input': input,
                            'weight': weight,
                            'offset_i': offset_i,
                ...
                block=tuple([512, 1, 1]),
                args=[n_i, gradOutput.data_ptr(), input.data_ptr(), weight.data_ptr(), offset_i.data_ptr(),
                      offset_j.data_ptr(), gradOffset_i.data_ptr()],
                stream=Stream
            )

            # beta grad
            n_j = gradOffset_j.nelement()
            cupy_launch('kernel_AdaCoF_updateGradBeta',
                        cupy_kernel('kernel_AdaCoF_updateGradBeta', intFilterSize, dilation, {
                            'gradLoss': gradOutput,
                            'input': input,
                            'weight': weight,
                            'offset_i': offset_i,
                ...
                block=tuple([512, 1, 1]),
                args=[n_j, gradOutput.data_ptr(), input.data_ptr(), weight.data_ptr(), offset_i.data_ptr(),
                      offset_j.data_ptr(), gradOffset_j.data_ptr()],
                stream=Stream
            )
