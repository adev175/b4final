...
from .RSTSCA_Encoder import ResBlock, RSTSCAGroup_Encoder
...
class RSTSCANet1(nn.Module):
    def __init__(self, n_inputs=4, joinType="concat", ks=5, dilation=1):
        ...
        self.encoder = RSTSCAGroup_Encoder(nf, n_inputs, window_size=ws, nh=nh)

        self.decoder = nn.Sequential(
            upSplit(upscale_factor=2),
            upSplit(upscale_factor=2),
            upSplit(upscale_factor=2),
        )
        ...
        def SmoothNet(inc, ouc):
            return torch.nn.Sequential(
                Conv_3d(inc, ouc, kernel_size=1, stride=1, padding=0, batchnorm=False),
                ResBlock(ouc, kernel_size=3),
            )

        self.smooth_ll = SmoothNet(384, nf[1])
        self.smooth_l = SmoothNet(192, nf[2])
        self.smooth = SmoothNet(96, nf[3])
    #Predict place
        self.predict = SynBlock(n_inputs, nf=nf[3], ks=ks, dilation=dilation, norm_weight=False)
    ...
    def forward(self, frames):
        ...
        #Stage 1 Decoder
        dx_3 = self.decoder[0](x_4)
        dx_3 = joinTensors(dx_3, x_3, type=self.joinType)
        dx_3 = self.smooth_ll(dx_3)
        #Stage 2 Decoder
        dx_2 = self.decoder[1](dx_3)
        dx_2 = joinTensors(dx_2, x_2, type=self.joinType)
        dx_2 = self.smooth_l(dx_2)
        #Stage 3 Decoder
        dx_1 = self.decoder[2](dx_2)
        dx_1 = joinTensors(dx_1, x_1, type=self.joinType)
        dx_1 = self.smooth(dx_1) #Final output of decoder or F: 2CxTxH/2xW/2
        #dx_1 = fea
        #Frame Synthesis
        outs = self.predict(dx_1, frames, x_0.size()[-2:])
        #SynBlock return Framet = [frame1, frame2]
...
class SynBlock(nn.Module):
    def __init__(self, n_inputs, nf, ks, dilation, norm_weight=True):
        ...
        self.moduleAdaCoF = adacof.FunctionAdaCoF.apply #output have size equal input frames
    ...
    def forward(self, fea, frames, output_size):
        ...
        for i in range(self.n_inputs):
            ...
            frame = F.interpolate(frames[i], size=weight.size()[-2:], mode='bilinear', align_corners=True)

            warp.append(
                occ * self.moduleAdaCoF(self.modulePad(frame), weight, alpha, beta, self.dilation)
            )
        # print("Length of warp:",len(warp))
        framets = [sum(warp[i:i+2]) for i in range(0, len(warp)-1)]
        # print("Shape of framets",framets)
        return framets
