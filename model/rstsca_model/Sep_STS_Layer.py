...
# ---------------------------------
class CALyer(nn.Module):
    def __init__(self, channels, reduction=16):
        super(CALyer, self).__init__()
        ...
        self.conv_fc = nn.Sequential(
            ...
            nn.Sigmoid()
        ) #MLP structure w 1 hidden layer as reduction block w default = 16
        #part of Squeeze and Excitation BLock

    def forward(self, x):
        y1 = self.avg_pool(x)
        y2 = self.max_pool(x)
        y = self.conv_fc(y1 + y2)
...
class RCABlock(nn.Module):
    def __init__(self, out_channels, reduction=16):
        ...
        self.ca = CALyer(out_channels, reduction)
...
class RSTSCABBasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.ca_block = RCABlock(out_channels=dim, reduction=dim // 4)
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i % 2 == 1:
                ca_inp = rearrange(res_input, 'b d h w c -> b c d h w')
                ca = self.ca_block(ca_inp)
                ca = rearrange(ca, 'b c d h w -> b d h w c')
                res_input = x * ca
                res_input += skip_connect
                x = res_input
